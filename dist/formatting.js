...
exports.IATA_MAP = exports.DESTINATIONS_LOOKUP = void 0;
exports.resolveCanonicalCity = resolveCanonicalCity;
exports.iataMatchesCity = iataMatchesCity;
exports.cityToIata = cityToIata;
exports.isBrazilianOrigin = isBrazilianOrigin;
exports.findDestinationImage = findDestinationImage;
exports.normalizeDatePairs = normalizeDatePairs;
exports.ensureHttps = ensureHttps;
exports.resolveLinkPrograma = resolveLinkPrograma;
exports.buildFormattedMessage = buildFormattedMessage;
exports.buildFormattedMessageCash = buildFormattedMessageCash;
exports.buildWhatsAppLinkCash = buildWhatsAppLinkCash;
...
exports.IATA_MAP = {
    ...
    // EUA
    "JFK": "Nova Iorque", "EWR": "Newark", "LGA": "Nova Iorque",
    ...
    "MAD": "Madri", "BCN": "Barcelona", "PMI": "Palma de Mallorca",
    ...
    "WAW": "Varsóvia", "PRG": "Praga", "BUD": "Budapeste", "VCE": "Veneza",
    ...
    "HAV": "Havana", "CUN": "Cancún", "GDL": "Guadalajara", "SJO": "San José", "AUA": "Aruba", "ADZ": "San Andrés", "BRC": "San Carlos de Bariloche",
...
const ALIASES = {
    ...
    "paris": "Paris",
    "london": "Londres",
    "new york": "Nova Iorque", "nova york": "Nova Iorque", "nueva york": "Nova Iorque", "nyc": "Nova Iorque",
    "newark": "Newark",
    "los angeles": "Los Angeles", "la": "Los Angeles",
    "fort lauderdale": "Fort Lauderdale", "fll": "Fort Lauderdale",
    ...
    "new delhi": "Nova Delhi", "delhi": "Nova Delhi", "nova delhi": "Nova Delhi", "nova deli": "Nova Delhi", "nova déli": "Nova Delhi",
    "mumbai": "Mumbai", "bombay": "Mumbai",
    "bangalore": "Bangalore", "bengaluru": "Bangalore",
    "colombo": "Colombo", "kathmandu": "Katmandu",
    "tel aviv": "Tel Aviv", "telaviv": "Tel Aviv",
    "cairo": "Cairo",
    "palma de mallorca": "Palma de Mallorca", "mallorca": "Palma de Mallorca",
    "venice": "Veneza", "veneza": "Veneza",
    ...
    "aruba": "Aruba",
    "bariloche": "San Carlos de Bariloche", "san carlos de bariloche": "San Carlos de Bariloche",
    "boa vista": "Boa Vista", "rio branco": "Rio Branco", "macapa": "Macapá", "macapá": "Macapá",
    "porto seguro": "Porto Seguro", "navegantes": "Navegantes",
...
const CITY_TO_IATA_MAP = {};
// Primeiro, preenche com todos (o último ganha)
for (const [iata, city] of Object.entries(exports.IATA_MAP)) {
    const n = norm(city);
    if (!CITY_TO_IATA_MAP[n])
        CITY_TO_IATA_MAP[n] = iata;
}
...
function resolveCanonicalCity(input) {
    ...
    if (!trimmed)
        return null;
    if (/^[A-Z]{3}$/i.test(trimmed) && exports.IATA_MAP[trimmed.toUpperCase()]) {
        return exports.IATA_MAP[trimmed.toUpperCase()];
    }
    ...
    const knownCities = new Set([
        ...Object.values(exports.IATA_MAP),
        ...Object.keys(exports.DESTINATIONS_LOOKUP),
...
const BRAZILIAN_CITIES_SET = new Set();
...
BRAZILIAN_IATA_CODES.forEach(code => {
    const city = exports.IATA_MAP[code];
    if (city)
        BRAZILIAN_CITIES_SET.add(norm(city));
});
function isBrazilianOrigin(origem) {
    if (!origem)
        return false;
    ...
    if (BRAZILIAN_CITIES_SET.has(inputNorm))
        return true;
    // Check aliases that resolve to Brazilian cities
    const aliasCity = ALIASES_NORM[inputNorm];
    if (aliasCity && BRAZILIAN_CITIES_SET.has(norm(aliasCity)))
        return true;
    // Partial match: if the input contains a known Brazilian city name
    for (const cityNorm of BRAZILIAN_CITIES_SET) {
        if (inputNorm.includes(cityNorm) || cityNorm.includes(inputNorm))
            return true;
    }
    return false;
}
function findDestinationImage(destino, overrides = {}) {
    if (!destino)
        return FALLBACK_IMAGE;
    const lookup = { ...exports.DESTINATIONS_LOOKUP, ...overrides };
    const input = destino.toString().trim();
    const inputNorm = norm(input);
    // Passo 1: IATA code
    if (/^[A-Z]{3}$/.test(input.toUpperCase())) {
        const iataCity = exports.IATA_MAP[input.toUpperCase()];
...
function countryToFlag(pais) {
    if (!pais)
        return "✈️";
    const key = Object.keys(COUNTRY_FLAGS).find(k => norm(k) === norm(pais) || norm(pais).includes(norm(k)) || norm(k).includes(norm(pais)));
 ...
 */
function normalizeDatePairs(datasIda, datasVolta, gapDays = 5) {
...
function resolveLinkPrograma(linkFromGpt, programaCanonical) {
    ...
    return milheiroHandler_1.PROGRAMA_LINKS[programaCanonical] || ensureHttps(gptLink) || 'https://www.smiles.com.br';
}
function buildFormattedMessage(data, milheiroPorPrograma = {}, isExecutiva = false) {
    ...
    const programa = (0, milheiroHandler_1.normalizeProgramaName)(programaRaw);
    ...
    const teaserBlock = isExecutiva
        ...
        : '';
    const header = `*${upperCity(origem)}* ✈️ *${upperCity(destino)}*\\n` +
        `Cia: ${cia}\\n` +
        `Programa: ${programa}\\n\\n`;
    const idaBlock = `*Ida:* 🛫\\n` +
    ...
    const totalBlock = `*Valor total:* R$ ${fmtMoneyBR(totalValue)} ${taxesSuffix}\\n\\n` +
...
function buildFormattedMessageCash(data, isExecutiva = false, hasDirectLink = false) {
    const origem = safeStr(data.origem, 'ORIGEM');
    const destino = safeStr(data.destino, 'DESTINO');
    const cia = safeStr(data.cia_aerea, 'CIA');
...
function buildWhatsAppLinkCash(data) {
...
function buildWhatsAppLink(data, milheiroPorPrograma = {}) {
