...
<body class="min-h-screen relative overflow-auto flex flex-col">
    ...
    <script>
        ...
        const PROGRAMA_ALIASES = ${JSON.stringify(milheiroHandler_1.PROGRAMA_ALIASES_DISPLAY)};
...
async function getDashboardData(req, res) {
    try {
        ...
        const { data: queueData, error: queueError } = await supabaseClient_1.supabase
            ...
            .limit(100);
        // 2. Fetch recent logs
        const { data: logsData, error: logsError } = await supabaseClient_1.supabase
            .from('api_logs')
        ...
        const queueList = (queueData || []).map((msg) => {
...
async function reprocessOffer(req, res) {
    ...
    try {
        ...
        if (images.length >= 2 && texts.length >= 1) {
            ...
            await supabaseClient_1.supabase
                ...
                .in('id', idsToReset);
            await (0, logger_1.logEvent)(phone, 'REPROCESS_INITIATED', 'Manual reprocess triggered via Dashboard', { idsToReset });
            ...
            await (0, webhookHandler_1.checkForCompleteOffer)(phone, 'Recuperado Dashboard');
